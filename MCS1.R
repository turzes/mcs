#  > MCSprocedure
function (Loss, alpha = 0.15, B = 5000, cl = NULL, ram.allocation = T, 
          statistic = "Tmax", k = NULL, min.k = 3, verbose = T) 
{
  time.start = Sys.time()
  Loss = as.matrix(Loss)
  M_start = ncol(Loss)
  ...
  repeat {
    ...
    col.names.d = do.call(c, lapply(1:M, function(x) paste(model.names[x], 
                                                           model.names[-x], sep = ".")))
    d = do.call(cbind, lapply(1:M, function(x) Loss[, x] - 
                                Loss[, -x]))
    colnames(d) = col.names.d
    d_ij_mean = colMeans(d)
    d_i_mean = sapply(1:M, function(x) sum(d_ij_mean[paste(model.names[x], 
                                                           model.names[-x], sep = ".")])/(M - 1))
    ...
    if (ncol(d) > 2) {
      ...
      if (!is.null(cl)) 
        k = max(na.omit(as.numeric(parSapply(cl, d_cut, 
                                             function(x) try(ar(x)$order, silent = T)))))
      if (is.null(cl)) 
        k = max(na.omit(as.numeric(sapply(d_cut, function(x) try(ar(x)$order, 
                                                                 silent = T)))))
    }
    else {
      k = ar(d[, 1])$order
    ...
    d_var_ij = matrix(colSums(((d_ij_avg_resampled - d_ij_mean)^2))/B, 
                      nrow = 1)
    colnames(d_var_ij) = names(d_ij_mean)
    TR = max((abs(d_ij_mean))/(d_var_ij^0.5))
    TM = max(d_i_mean/(d_b_i_var^0.5))
    Tb_R = sapply(1:B, function(i) max(abs(d_ij_avg_resampled[i, 
                                                              ] - d_ij_mean)/(d_var_ij^0.5)))
    Tb_M = sapply(1:B, function(i) max((d_b_i_mean[i, ] - 
                                          d_i_mean)/(d_b_i_var^0.5)))
    ...
    v_i_R = sapply(1:M, function(x) max(TR_all[1, paste(model.names[x], 
                                                        model.names[-x], sep = ".")]))
    ...
    for (i in 1:M) {
      ...
      if (i == 1) {
        ...
        remove = do.call(c, lapply(1:(i - 1), function(x) which(gsub(model.names[v_i_R_order[x]], 
                                                                     "", combine.names) != combine.names)))
        model_temp = combine.names[-remove]
      }
      if (i < M) {
        TR_temp = max((abs(d_ij_mean[model_temp]))/(d_var_ij[1, 
                                                             model_temp]^0.5))
    ...
    matrix_show = matrix(, M, 7, dimnames = list(model.names, 
  ...
  out = new("SSM", show = matrix_show, Info = list(model.names = model.names, 
